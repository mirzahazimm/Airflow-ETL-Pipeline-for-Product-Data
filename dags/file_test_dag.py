...
from airflow.providers.microsoft.mssql.hooks.mssql import MsSqlHook #hook to establish connection to db
...
def load_src_data(tbl_dict: dict):
    ...
    engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}') # sqlalchemy
    all_tbl_name = []
...
with DAG(dag_id="product_etl_dag",schedule_interval="0 9 * * *", start_date=datetime(2024, 11, 1),catchup=False,  tags=["product_model"]) as dag:
